import { BaseProvider, ProviderMatch, RequestData, ProviderConfig } from '../base/base-provider'
...
export class ProviderRegistry {
  ...
  async analyze(request: RequestData, options: {
    ...
    try {
      ...
      const bestMatch = limitedMatches.length > 0 ? limitedMatches[0] : null
  ...
  private clearCacheForProvider(providerId: string): void {
    ...
    this.analysisCache.forEach((result, key) => {
      if (result.matches.some(match => match.provider.getId() === providerId)) {
